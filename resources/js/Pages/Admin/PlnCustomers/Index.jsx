...
export default function PlnCustomersIndex({ auth, plnCustomers }) {
  return (
    <AuthenticatedLayout auth={auth}>
      <Head title="Pelanggan PLN" />
      <div className="flex-1 flex flex-col gap-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
          <div>
            <h2 className="text-xl md:text-2xl font-black tracking-tight">
              Pelanggan <span className="text-primary italic">PLN</span>
            </h2>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
              Kelola data pelanggan listrik PLN
            </p>
          </div>
        ...
        <Card className="bg-white dark:bg-slate-900 border-none shadow-sm rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-sm font-bold">Daftar Pelanggan PLN</CardTitle>
          </CardHeader>
          <CardContent>
            {!plnCustomers || (plnCustomers.data || []).length === 0 ? (
              <div className="text-center py-12">
                <Users className="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
                <p className="text-[11px] text-slate-500 dark:text-slate-400">Tidak ada data pelanggan</p>
              </div>
            ) : (
              <>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-slate-100 dark:border-slate-800">
                        ...
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 pb-3 pr-4">Nama Pelanggan</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 pb-3 pr-4">Nomor Meter</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 pb-3 pr-4">Daya</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 pb-3 pr-4">Alamat</th>
                        <th className="text-[10px] font-black uppercase tracking-widest text-slate-400 pb-3">Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(plnCustomers.data || []).map((customer) => (
                        <motion.tr
                          key={customer.id}
                        ...
                        >
                          <td className="py-3 pr-4 text-xs font-bold">{customer.id}</td>
