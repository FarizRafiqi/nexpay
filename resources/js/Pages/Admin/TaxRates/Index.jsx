        ...
        <AuthenticatedLayout auth={auth}>
            ...
            <div className="flex-1 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl md:text-2xl font-black tracking-tight">Tarif Pajak</h2>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Kelola tarif pajak berdasarkan tipe</p>
                    </div>
                </div>
                    ...
                    <CardContent className="p-0">
                        {taxRates.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                                ...
                                <p className="text-sm font-medium">Belum ada tarif pajak</p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-slate-100 dark:border-slate-800">
                                            ...
                                            <th className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Tipe Pajak</th>
                                    ...
                                    <tbody>
                                        {taxRates.map((rate) => (
                                            <tr key={rate.id} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                <td className="px-4 py-3 text-xs font-mono text-slate-500">#{rate.id}</td>
                                                <td className="px-4 py-3">
                                                    {rate.tax_type ? (
                                                        <Badge variant="secondary" className="text-[10px] font-bold">{rate.tax_type.nama}</Badge>
                                                    ) : (
